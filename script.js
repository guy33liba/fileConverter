...
function openFilePicker(event) {
  ...
  if (typeof elements.fileInput.showPicker === "function") {
    try {
      ...
      // Fall back to click() when the picker API is unavailable or blocked.
    }
  }
  ...
  if (isPdf) {
    ...
    try {
      ...
      setStatus(
        50,
        "Ready",
        `${pdfDoc.numPages} page${pdfDoc.numPages > 1 ? "s" : ""} loaded. Select output format.`,
      );
...
async function convertPdfToImages(outputType) {
  ...
  try {
    ...
    setStatus(
      80,
      "Converting",
      `Creating ${outputType.replace("pdf-to-", "").toUpperCase()} file...`,
    );
...
function showTextPreview(text) {
  ...
  pre.style.cssText =
    "max-height: 300px; overflow-y: auto; font-size: 12px; background: #f5f5f5; padding: 12px; border-radius: 8px; white-space: pre-wrap; word-wrap: break-word; max-width: 100%;";
